...
### Server
- Fix: pgroonga検索で`-`や`(`等の記号を含むキーワードだけで検索すると構文エラーになり検索が失敗する問題を修正

## 2026.9.0-art+3.0

### Client
- Fix: お知らせへのリアクションが「リアクションする際確認する」設定を無視して常に確認無しで即反映されていた問題を修正
- Fix: 他ユーザーがお知らせへリアクションした際に、リアルタイムで反映されない問題を修正

### Server
- Fix: 特定ユーザー宛のお知らせに、宛先本人を含め誰もリアクションできないように
- Feat: お知らせ1件に付けられるリアクションの種類数の上限をロールポリシーで設定できるように
...
### Client
- Fix: お知らせリアクションで既に付けたカスタム絵文字を再選択すると重複する問題を修正

## 2026.7.0-art+2.1

### Client
- Fix: お知らせリアクションで同じリアクションを複数回選ぶとUI上で重複する問題を修正
...
- Fix: ノート内のセンシティブ画像のぼかしが設定によって消える問題を修正
	- CWを開いた後のセンシティブ画像にもぼかしが表示されるように修正
	- 複数のセンシティブ画像が1つにまとまって表示される問題を修正
- Fix: CWを閉じた状態や非表示の要素内で描画されたBlurhashが表示されない問題を修正
